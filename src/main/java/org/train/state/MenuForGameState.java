...
public class MenuForGameState extends BasicGameState {
    ...
    public void render(GameContainer container, StateBasedGame game, Graphics g) throws SlickException {
        ...
        if (levelIndex < levelPackages.get(packageIndex).getLevelNames().size()) {
            g.drawString(showingText, width - ubuntuMedium.getWidth(showingText) * 1.1f,
                    height - ubuntuMedium.getHeight(showingText) * 1.1f);
        }

        g.setColor(new Color(0, 0.5f, 0));
        g.drawString(progressText, width - ubuntuMedium.getWidth(progressText) * 1.1f, height * 16 / 30);
    }
    ...
    private void setProgressText() {
        ...
        progressText = String.format("%4$s %1$d %3$s %2$d", this.progress.getLastAvailableLevelIndex(packageIndex),
                size, translator.translate((size > 1 && size < 5) ? "Alternative.Of" : "Of"),
    ...
    private void createPlayButton(final StateBasedGame game) {
        ...
        this.playBtn = buttonFactory.setDefaultColor(Color.white).setDisabledColor(Color.darkGray)
                .setOverColor(Color.red).setDefaultText(translator.translate("play")).setDefaultFont(ubuntuMedium)
                .setListener(new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        try {
        ...
        this.playBtn.setPosition(new org.newdawn.slick.geom.Point(width / 2 - this.playBtn.getWidth() / 2,
                (int) (height - this.playBtn.getHeight() * 1.1f)));
    }

    private void createArrowButtons() {
        ButtonFactory buttonFactory = this.container.getComponent(ButtonFactory.class);
        buttonFactory.setDefaultFont(this.ubuntuLarge).setNormalColor(Color.white).setOverColor(Color.red)
                .setDisabledColor(Color.darkGray);
        ...
        this.packageArrowLeft.setPosition(new org.newdawn.slick.geom.Point(width * 1 / 4, height * 1 / 3));

        this.packageArrowRight = buttonFactory.setDefaultText(">").setListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                showNextPackage();
            }
        }).createButton();
        this.packageArrowRight.setPosition(
                new org.newdawn.slick.geom.Point(width * 3 / 4 - this.ubuntuLarge.getWidth(">"), height * 1 / 3));
        ...
        this.levelArrowLeft.setPosition(new org.newdawn.slick.geom.Point(width * 1 / 4, height * 3 / 4));
        ...
        this.levelArrowRight.setPosition(
                new org.newdawn.slick.geom.Point(width * 3 / 4 - this.ubuntuLarge.getWidth(">"), height * 3 / 4));
    }
